...
class Dumbledore extends Bot {
  constructor(settings) {
    settings.token = settings.token || 'xoxb-248266143441-zgzDCId1tMbBG7GKaToXsUi2';
    settings.name = settings.name || 'bot1';
  ...
  _awardPoints(originalMessage) {
    debugger
      ...
      if (originalMessage.text.toLowerCase().indexOf('gryffindor') > -1) {
        global.db.run('UPDATE houses SET points = points + ? WHERE house = "gryffindor"', points);
        global.db.run('UPDATE students SET points_given = (points_given + ?) WHERE user_id = ?', points, originalMessage.user);
        ...
        global.db.run('UPDATE houses SET points = points + ? WHERE house = "hufflepuff"', points);
        global.db.run('UPDATE students SET points_given = (points_given + ?) WHERE user_id = ?', points, originalMessage.user);
        ...
        global.db.run('UPDATE houses SET points = points + ? WHERE house = "ravenclaw"', points);
        global.db.run('UPDATE students SET points_given = (points_given + ?) WHERE user_id = ?', points, originalMessage.user);
        ...
        global.db.run('UPDATE houses SET points = points + ? WHERE house = "slytherin"', points);
        global.db.run('UPDATE students SET points_given = (points_given + ?) WHERE user_id = ?', points, originalMessage.user);
        ...
        global.db.get('SELECT user_id, username, house FROM students WHERE user_id = ?', student, function (err, record) {
          ...
          if (record !== undefined) {
            ...
            global.db.run('UPDATE students SET points_earned = (points_earned + ?) WHERE user_id = ?', points, student);
          }
        });
      }
  }
    ...
    if (originalMessage.attachments[0].pretext !== undefined) {
      if (originalMessage.attachments[0].pretext.indexOf('New comment by ') > -1) {
        ...
        if (originalMessage.attachments[0].text.indexOf(':+1:') > -1) {
          global.db.get('SELECT * FROM students WHERE github_name = ?', gitUser, function (err, record) {
            if (err) {
              return console.error('DATABASE ERROR', err);
            } if (record != undefined) {
              global.db.run('UPDATE students SET points_earned = (points_earned + ?) WHERE user_id = ?', 5, record.user_id);
              global.db.run('UPDATE houses SET points = points + ? WHERE house = ?', 5, record.house);
            }
          });
        }
      }
