...
const SlackBot = require('./helper/slackBot');//slackBot
...
class Dumbledore {
  ...
  isReactionEvent(message) {
    if (typeof message.item === 'undefined') return;
    const isReactionMessage = message.type === 'reaction_added' || message.type === 'reaction_removed';
    const isScoreReaction = (message.reaction === '100') || (message.reaction === '+1') || (message.reaction === 'grinning') || (message.reaction === 'heart');
