...
class Dumbledore {
  ...
  async _loadBotUser() {
    const users = await slackBot.getUserList();

    [this.user] = users.members.filter((user) => {
  ...
  _welcomeMessage() {
    slackBot.postMessageToChannel(this.channels[0].name, OUTPUT.SAY_HELLO, { as_user: true });
  }
