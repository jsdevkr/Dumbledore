...
async function helpMessage(originalMessage) {
  ...
  await slackBot.announcePlainString(originalMessage, pointTo);

  // for points from
  const pointFrom = `\`Num\` ${POINTS_FROM.CALL} \`house name\``;
  await slackBot.announcePlainString(originalMessage, pointFrom);
  ...
  Object.keys(PROFESSOR).forEach(async k => {
    if (k !== 'CALL') {
      ...
      await slackBot.announcePlainString(originalMessage, str);
    }
  });
}
