...
function awardPoints(originalMessage) {
  ...
  if (originalMessage.text.toLowerCase().indexOf('gryffindor') > -1) {
    ...
    getPointsFromDatabase.call(this, originalMessage, 'gryffindor', slackBot.awardPointsCallback);
    ...
    this.db.run('UPDATE students SET points_given = (points_given + ?) WHERE user_id = ?', points, originalMessage.user);
    getPointsFromDatabase.call(this, originalMessage, 'hufflepuff', slackBot.awardPointsCallback);
    ...
    this.db.run('UPDATE students SET points_given = (points_given + ?) WHERE user_id = ?', points, originalMessage.user);
    getPointsFromDatabase.call(this, originalMessage, 'ravenclaw', slackBot.awardPointsCallback);
    ...
    this.db.run('UPDATE students SET points_given = (points_given + ?) WHERE user_id = ?', points, originalMessage.user);
    getPointsFromDatabase.call(this, originalMessage, 'slytherin', slackBot.awardPointsCallback);
