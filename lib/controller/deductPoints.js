...
function deductPoints(originalMessage) {
  ...
  if (originalMessage.text.toLowerCase().indexOf('gryffindor') > -1) {
    ...
    getPointsFromDatabase.call(this, originalMessage, 'gryffindor', slackBot.deductPointsCallback);
    ...
    getPointsFromDatabase.call(this, originalMessage, 'hufflepuff', slackBot.deductPointsCallback);
    this.db.run('UPDATE students SET points_taken = (points_taken + ?) WHERE user_id = ?', points, originalMessage.user);
    ...
    getPointsFromDatabase.call(this, originalMessage, 'ravenclaw', slackBot.deductPointsCallback);
    this.db.run('UPDATE students SET points_taken = (points_taken + ?) WHERE user_id = ?', points, originalMessage.user);
    ...
    getPointsFromDatabase.call(this, originalMessage, 'slytherin', slackBot.deductPointsCallback);
    this.db.run('UPDATE students SET points_taken = (points_taken + ?) WHERE user_id = ?', points, originalMessage.user);
