...
async function getAllHouseScores(originalMessage, callback) {
  const channel = await slackBot.getChannelById(originalMessage.channel);

  slackBot.postMessageToChannel(channel.name, 'The House Points are: \n', { as_user: true }, () => {
...
function resetTheScores(originalMessage, callback) {
  ...
  callback(originalMessage, OUTPUT.RESET_SCORE);
}

function welcomeMessageManual(originalMessage) {
  slackBot.announcePlainString(originalMessage, OUTPUT.SAY_HELLO);
}
...
async function convertToUserName(key) {
  const users = await slackBot.getUsers();
...
function greetNewStudent(originalMessage, student, house) {
  slackBot.announcePlainString(originalMessage, OUTPUT.canIJoin(student, house));
}
    ...
    if (respond !== undefined) {
      respond.find((dbUser) => {
        if (dbUser.user_id === student) {
          // slackBot.announcePlainString(originalMessage, studentUsername + ' You are already in a house. Once You have been sorted in that house you shall remain. But fear not, if you give it a chance you will see there is much to gain.', bot);
...
function obliviate(originalMessage) {
  ...
  if (student === convertToUserName(originalMessage.user)) {
    this.db.run('DELETE FROM students WHERE user_id = ?', student, (err) => {
      ...
      slackBot.announcePlainString(originalMessage, OUTPUT.OBLIVIATE);
    });
  }
}
...
function studentStats(originalMessage) {
  ...
  if (convertToUserName(student) !== 'dumbledore') {
    this.db.get('SELECT * FROM students WHERE user_id = ?', student, (err, record) => {
      ...
      if (record !== undefined) {
        slackBot.announcePlainString(originalMessage, OUTPUT.TELL_ME_ABOUT.PERSON.student(record));
      } else {
        slackBot.announcePlainString(originalMessage, OUTPUT.TELL_ME_ABOUT.PERSON.NOT_FOUND);
      }
    });
  } else {
    slackBot.announcePlainString(originalMessage, OUTPUT.TELL_ME_ABOUT.PERSON.DUMBLEDORE);
  }
}
...
function bestStudent(originalMessage) {
  Object.values(HOUSE).forEach((house) => {
    this.db.get('SELECT * FROM students WHERE house = ? ORDER BY points_earned DESC', house, (err, record) => {
      ...
      if (record !== undefined) {
        slackBot.announcePlainString(originalMessage, OUTPUT.getBestStudent(record));
      }
    });
  });
  slackBot.announcePlainString(originalMessage, OUTPUT.ABOUT_FROGS);
}
...
function worstStudent(originalMessage) {
  Object.values(HOUSE).forEach((house) => {
    this.db.get('SELECT * FROM students WHERE house = ? ORDER BY points_taken DESC', house, (err, record) => {
      ...
      if (record !== undefined) {
        slackBot.announcePlainString(originalMessage, OUTPUT.getMeanestStudent(record));
      }
    });
  });
}
...
function getAllStudentsFromHouse(originalMessage, house) {
  this.db.all('SELECT * FROM students WHERE house = ? ', house, (err, record) => {
    ...
    slackBot.announcePlainString(originalMessage, OUTPUT.TELL_ME_ABOUT.fromHouse(house, record));
  });
}
...
function tellMeAbout(originalMessage, house) {
  ...
  if (text.includes('@')) {
    ...
    slackBot.announcePlainString(originalMessage, OUTPUT.TELL_ME_ABOUT.JASON_MOM);
  }
}
...
function explainSorting(originalMessage) {
  slackBot.announcePlainString(originalMessage, OUTPUT.START_SORTING);
}
...
function saveGitName(originalMessage) {
  ...
  this.db.run('UPDATE students SET github_name = ? WHERE user_id = ?', gitName, user, (err) => {
    ...
    slackBot.announcePlainString(originalMessage, OUTPUT.saveGitName(convertToUserName(user), gitName));
  });
}
...
function replyWithDumbledore(originalMessage) {
  ...
  const parseCase = {
    [INPUT.PROFESSOR.GET_WINNER]: () => getAllHouseScores.call(this, originalMessage, slackBot.getAllHousePointsCallback),
    [INPUT.PROFESSOR.RESET_SCORE]: () => resetTheScores.call(this, slackBot.announcePlainString),
    [INPUT.PROFESSOR.SAY_HELLO]: () => welcomeMessageManual(originalMessage),
    ...
    [INPUT.PROFESSOR.START_SORTING]: () => explainSorting(originalMessage),
