...
function replyWithDumbledore(originalMessage) {
  ...
  const parseCase = {
    [INPUT.PROFESSOR.GET_WINNER]: () => getAllHouseScores.call(self, originalMessage, botCommand.getAllHousePointsCallback),
    [INPUT.PROFESSOR.RESET_SCORE]: () => resetTheScores.call(self, botCommand.announcePlainString),
    [INPUT.PROFESSOR.SAY_HELLO]: () => welcomeMessageManual.call(self, originalMessage),
    [INPUT.PROFESSOR.CAN_I_JOIN]: () => addStudentToHouse.call(self, originalMessage, null, null /*self._studentJoinedHouse*/),
    [INPUT.PROFESSOR.SORTING_HAT]: () => rollTheDice.call(self, originalMessage),
    [INPUT.PROFESSOR.OBLIVIATE]: () => obliviate.call(self, originalMessage),
    [INPUT.PROFESSOR.GET_BEST_STUDENT]: () => bestStudent.call(self, originalMessage),
    [INPUT.PROFESSOR.GET_MEANEST_STUDENT]: () => worstStudent.call(self, originalMessage),
    [INPUT.PROFESSOR.TELL_ME_ABOUT]: () => tellMeAbout.call(self, originalMessage, house),
    [INPUT.PROFESSOR.START_SORTING]: () => explainSorting.call(self, originalMessage),
    [INPUT.PROFESSOR.GET_HOGWARTS_ROSTER]: () => getAllStudents.call(self, originalMessage),
    [INPUT.PROFESSOR.SAVE_GIT_NAME]: () => saveGitName.call(self, originalMessage),
    [INPUT.PROFESSOR.SORT_REST]: () => forceSortRemaining.call(self, originalMessage)
  }
    ...
    if (notAlreadyStudent === true) {
      ...
      Object.values(HOUSE).filter(t => text.includes(t) || house === key).some(v => {
        this.db.run('INSERT INTO students (user_id, username, house) VALUES (?, ?, ?)', student, studentUsername, v, (err, respond) => {
          ...
          greetNewStudent.call(self, originalMessage, studentUsername, v);
        });
        return true;
      })
    }
...
function tellMeAbout(originalMessage, house) {
  ...
  if (text.includes('@')) {
    ...
    botCommand.announcePlainString(originalMessage, OUTPUT.TELL_ME_ABOUT.JASON_MOM, self);
  } else {
    return;
  }
}
