...
class SlackBot extends Bot {
  constructor({ token, name }) {
    ...
    this.deductPointsCallback = this.deductPointsCallback.bind(this);
  ...
  async awardPointsCallback(originalMessage, result) {
    const channel = await this.getChannelById(originalMessage.channel);

    await this.postMessageToChannel(channel.name, OUTPUT.pointTo(result.attributes), { as_user: true });
  }
